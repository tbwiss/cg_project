	...
	this.createSportInSquare	= function(){
		...
		for(var blockZ = 0; blockZ < typeOfBlock.length; blockZ++) {
		    ...
		    for(var blockX = 0; blockX < block.length; blockX++) {
		         ...
		         //		var geometry = new THREE.CubeGeometry( 30, 0.1, 20 );
		         //		var mesh = THREE.Mesh(geometry);
		         		
		         		/*
		         		var texture	= THREE.ImageUtils.loadTexture( "../images/soccer_ground.jpg" );
						...
						var materials = [
						    materialB,        // Left side
						    materialB,       // Right side
						    materialA,         // Top side
						    materialA,      // Bottom side
						    materialB,       // Front side
						    materialB         // Back side
						];*/
						
				//		for(var ig = 0; ig < mesh.geometry.faces.length; ig++ ) {
				//			mesh.geometry.faces[ig].color.set(0x6C4941 );
				//		}
						
						//var geometry = new THREE.CubeGeometry(30, 0.1, 20, 1, 1, 1, materials);
						//var sportGround = new THREE.Mesh(geometry);
						
						//var sportGround = new THREE.Mesh( geometry, materialA );
						...
						var sportGround = new THREE.Mesh(new THREE.PlaneGeometry(30, 20), material);
						sportGround.material.side = THREE.DoubleSide;
											
						//var sportGround = new THREE.Mesh( geometry, material );*/
						
					//	sportGround.position.y = 0.1;
				//		sportGround.position.x	= (blockX+0.5-nBlockX/2)*blockSizeX;
				//		sportGround.position.z	= (blockZ+0.5-nBlockZ/2)*blockSizeZ;
						
						
						//THREE.GeometryUtils.merge( sportGeometry, mesh );
		...
		for(var blockZ = 0; blockZ < typeOfBlock.length; blockZ++) {
		    ...
		    for(var blockX = 0; blockX < block.length; blockX++) {
				...
				/*	
					...
					californiaPts.push( new THREE.Vector2 ( 30,  0 ) );
					californiaPts.push( new THREE.Vector2 ( 30,  30 ) );
					californiaPts.push( new THREE.Vector2 ( 0,  0 ) );
				/*	californiaPts.push( new THREE.Vector3 ( 450, 0.1, 300 ) );
					californiaPts.push( new THREE.Vector3 ( 392, 0.1, 392 ) );
					californiaPts.push( new THREE.Vector3 ( 266, 0.1, 438 ) );
					californiaPts.push( new THREE.Vector3 ( 190, 0.1, 570 ) );
					californiaPts.push( new THREE.Vector3 ( 190, 0.1, 600 ) );
					californiaPts.push( new THREE.Vector3 ( 160, 0.1, 620 ) );
					californiaPts.push( new THREE.Vector3 ( 160, 0.1, 650 ) );
					californiaPts.push( new THREE.Vector3 ( 180, 0.1, 640 ) );
					californiaPts.push( new THREE.Vector3 ( 165, 0.1, 680 ) );
					californiaPts.push( new THREE.Vector3 ( 150, 0.1, 670 ) );
					californiaPts.push( new THREE.Vector3 (  90, 0.1, 737 ) );
					californiaPts.push( new THREE.Vector3 (  80, 0.1, 795 ) );
					californiaPts.push( new THREE.Vector3 (  50, 0.1, 835 ) );
					californiaPts.push( new THREE.Vector3 (  64, 0.1, 870 ) );
					californiaPts.push( new THREE.Vector3 (  60, 0.1, 945 ) );
					californiaPts.push( new THREE.Vector3 ( 300, 0.1, 945 ) );
					californiaPts.push( new THREE.Vector3 ( 300, 0.1, 743 ) );
					californiaPts.push( new THREE.Vector3 ( 600, 0.1, 473 ) );
					californiaPts.push( new THREE.Vector3 ( 626, 0.1, 425 ) );
					californiaPts.push( new THREE.Vector3 ( 600, 0.1, 370 ) );
					californiaPts.push( new THREE.Vector3 ( 610, 0.1, 320 ) );
				//	for( var i = 0; i < californiaPts.length; i ++ ) californiaPts[ i ].multiplyScalar( 0.1 );
					var californiaShape = new THREE.Shape( californiaPts );
					var geometry = new THREE.ShapeGeometry( californiaShape );
					
					
					
					var mesh = new THREE.Mesh( geometry, new THREE.MeshPhongMaterial( { color: 0xf08000, side: THREE.DoubleSide } ) );
					*/
					var points = [0,1 , 1,0, 0,-1, -1,0, ];
					var splinePoints = getCurvePoints(points, 0.5, 25, true);
					
					var geometry = new THREE.ShapeGeometry(  new THREE.Shape(splinePoints) );
					/*
					shape.moveTo( -25, -25 );
					shape.bezierCurveTo(-20,20,20,20,15,2);
					shape.moveTo( 15,2 );
					shape.bezierCurveTo(15,2,30,30,-25, -25);
					...
					mesh.geometry.applyMatrix( new THREE.Matrix4().makeRotationX( ( 270 * (Math.PI/180) ) ) );
					...
					mesh.position.y = 3;
					mesh.position.x	= (blockX+0.5-nBlockX/2)*blockSizeX;
					mesh.position.z	= (blockZ+0.5-nBlockZ/2)*blockSizeZ;
					
					
					THREE.GeometryUtils.merge( lakeGeometry, mesh );	
